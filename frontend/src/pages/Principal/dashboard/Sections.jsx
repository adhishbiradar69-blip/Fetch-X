/* The five .lvl sections of the dashboard (School, Subject, Class, Student,
   Teachers — v5 designer update) — markup mirrors dashboard.html. */
...
import { ATT_TABS, CMP_METRICS, initials, pct } from './util';

/* ---------------------------------------------- small building blocks */
export function SectionHead({ title, sub, tag }) {
  return (
...
export function SectionSchool({
  ...
  cmpClasses, cmpMetric, onCmpMetric, cmpLoading, onOpenClass,
}) {
  ...
  const cmpItems = useMemo(
    () => (cmpClasses || [])
      .map((c) => ({
        key: c.id,
        label: c.name,
        tip: `Class ${c.name}${c.grade ? ` · Grade ${c.grade}` : ''}`,
        val: Math.round(c[cmpMetric === 'avg' ? 'avg' : cmpMetric === 'attendance' ? 'attendance_pct' : 'tasks_pct'] ?? 0),
      }))
      .sort((a, b) => b.val - a.val || a.label.localeCompare(b.label)),
    [cmpClasses, cmpMetric],
  );
  ...
  return (
    <section className={`lvl lvl-1 first rv${secIn ? ' in' : ''}`} id="schoolSec" ref={secRef}>
      ...
      <div className="card chart-card fade">
        <div className="chead">
          <span className="label">CLASS COMPARISON · {CMP_METRICS[cmpMetric]} · {(cmpClasses || []).length} CLASSES · CLICK A BAR TO OPEN</span>
          <div className="ctabs">
            {Object.entries(CMP_METRICS).map(([k, label]) => (
              <button key={k} type="button" className={`gtab${k === cmpMetric ? ' active' : ''}`} onClick={() => onCmpMetric(k)}>{label}</button>
            ))}
          </div>
        </div>
        <div className="cbody">
...
/* =========================================================== 02 SUBJECT */
export function SectionSubjects({ termAvg }) {
  ...
  return (
    <section className={`lvl lvl-2 rv${secIn ? ' in' : ''}`} id="subjectSec" ref={secRef}>
      <SectionHead
        title="Subject Level"
        sub="How each subject performs across the whole school, term by term."
      ...
      {!termAvg ? (
        ...
        <div className="subject-grid">
          {subjects.map((s) => (
            <div className="card s-card fade" key={s.name}>
...
function ClassCard({ c, total, onOpen }) {
  return (
    ...
    >
      ...
      <div className="s-body">
        <div className="chips">
          <Chip t="STUDENTS" n={c.students ?? '—'} />
          <Chip t="ATT" n={`${pct(c.attendance_pct)}%`} />
          <Chip t="TASK" n={`${pct(c.tasks_pct)}%`} />
        </div>
        <div className="avg-wrap">
          <span className="avg-label">AVERAGE</span>
  ...
  return (
    <section className={`lvl lvl-6 rv${secIn ? ' in' : ''}`} id="teacherSec" ref={secRef}>
      <SectionHead
        ...
        tag={teachers != null
          ? `${teachers.length} TEACHERS · CLICK FOR REPORT`
          : 'FACULTY'}
      />
      <div className="searchbar fade">
        ...
        <span className="count">
          {teachers != null
            ? `${rows.length} ${ql ? `MATCH${rows.length === 1 ? '' : 'ES'}` : 'TEACHERS'}`
            : '…'}
        </span>
      </div>
  ...
  return (
    <section className={`lvl lvl-4 rv${secIn ? ' in' : ''}`} id="studentSec" ref={secRef}>
      <SectionHead
        ...
        tag={total != null ? `${total} STUDENTS · CLICK FOR REPORT CARD` : 'CLICK A STUDENT FOR REPORT CARD'}
      />
      <div className="searchbar fade">
        <Search strokeWidth={2} />
        <input
        ...
        />
        <span className="count">
          {total != null ? `${total} ${query ? `MATCH${total === 1 ? '' : 'ES'}` : 'STUDENTS'}` : '…'}
        </span>
      </div>
      <div className="st-list-wrap fade">
